...
interface DiscountsCreateProps {
  stickerTypes: string[];
  sizes: Size[];
}

export default function DiscountsCreate({ stickerTypes, sizes }: DiscountsCreateProps) {
  ...
  return (
    <AdminLayout>
      ...
      <div className="space-y-6 max-w-xl">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-900">Tambah Diskaun</h2>
          <p className="admin-page-copy">Cipta diskaun baharu untuk sticker.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="admin-flat-card p-6 space-y-5">
          <div>
            ...
            <input id="name" type="text" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="cti: Promosi Mirrorcote Bulat" className="w-full rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-900 outline-none transition focus:border-brand-300 focus:ring-2 focus:ring-brand-100" />
