...
<div class="max-w-4xl">
    <div class="mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500 font-medium">Kemaskini maklumat dan visual untuk design <span class="text-indigo-600 font-bold">"{{ $design->name }}"</span>.</p>
        </div>
                ...
                <form method="post" action="{{ route('admin.designs.update', $design) }}" enctype="multipart/form-data" class="p-8 space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $design->name) }}" class="block w-full rounded-2xl border-0 py-3 px-4 text-slate-900 font-bold tracking-wider ring-1 ring-inset ring-slate-200 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-600 text-sm leading-6 shadow-sm" required>
                        </div>
                    </div>

                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2 pl-1">Deskripsi Design</label>
                        <textarea name="description" rows="3" class="block w-full rounded-2xl border-0 py-3 px-4 text-slate-900 font-bold tracking-wide ring-1 ring-inset ring-slate-200 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-600 text-sm leading-6 shadow-sm">{{ old('description', $design->description) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        <div class="flex h-6 items-center">
                            <input id="is_active" name="is_active" type="checkbox" value="1" class="h-5 w-5 rounded-lg border-slate-300 text-indigo-600 focus:ring-indigo-600 cursor-pointer" {{ old('is_active', $design->is_active) ? 'checked' : '' }}>
                        </div>
