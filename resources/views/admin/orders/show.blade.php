...
<div class="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div class="flex items-center gap-4">
        <a href="{{ route('admin.orders.index') }}" class="group h-12 w-12 flex items-center justify-center rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 hover:ring-indigo-300 hover:bg-slate-50 transition-all">
            <svg class="h-5 w-5 text-slate-400 group-hover:text-indigo-600 transition-colors" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0l7.5-7.5M3 12h18" /></svg>
        </a>
        <div>
            <h2 class="text-3xl font-black text-slate-900 tracking-tight uppercase leading-none mb-1">Perincian Tempahan</h2>
            <div class="flex items-center gap-3">
                <span class="text-xs font-black text-indigo-600 italic tracking-widest bg-indigo-50 px-3 py-1 rounded-full ring-1 ring-indigo-200">#{{ $order->order_no }}</span>
...
<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
    ...
    <div class="lg:col-span-2 space-y-8 animate-in fade-in slide-in-from-left-8 duration-700">
        <!-- Customer Info Card -->
        <div class="bg-white rounded-[2.5rem] shadow-sm ring-1 ring-slate-200 overflow-hidden transform transition-all duration-500 hover:shadow-xl hover:shadow-indigo-500/5">
            <div class="px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex items-center gap-4">
                <div class="h-12 w-12 rounded-2xl bg-white shadow-sm flex items-center justify-center text-indigo-600 ring-1 ring-slate-200">
            ...
            <div class="p-10">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <div class="space-y-6">
                        <div class="group">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block group-hover:text-indigo-500 transition-colors">Nama Penuh</span>
                            <p class="text-2xl font-black text-slate-900 capitalize tracking-tight leading-none">{{ $order->customer_name }}</p>
                        </div>
                        
                        <div class="group">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block group-hover:text-indigo-500 transition-colors">Nombor Telefon</span>
                            <div class="flex items-center gap-4">
                                <p class="text-xl font-black text-indigo-600 tracking-wider leading-none italic">{{ $order->customer_phone }}</p>
                        ...
                        <div class="group">
                            <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block group-hover:text-indigo-500 transition-colors">Alamat Lengkap</span>
                    ...
                    <div class="space-y-6">
                        <div class="group">
                             ...
                             <div class="inline-flex items-center gap-3 px-6 py-4 bg-indigo-50/50 rounded-2xl border border-indigo-100">
                                <svg class="h-5 w-5 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M21 7.5l-9-5.25L3 7.5m18 0l-9 5.25m9-5.25v9l-9 5.25M3 7.5l9 5.25M3 7.5v9l9 5.25m0-5.25v9" /></svg>
                        ...
                        @if($order->payment_receipt_path)
                            <div class="pt-4">
                                <a href="{{ asset('storage/'.$order->payment_receipt_path) }}" target="_blank" class="inline-flex w-full items-center justify-center gap-3 rounded-2xl bg-slate-900 px-6 py-4 text-sm font-black text-white hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 group/receipt">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        ...
                        <tr class="group/row hover:bg-slate-50 transition-all">
                            ...
                            <td class="whitespace-nowrap px-3 py-7 text-sm text-center">
                                <span class="inline-flex items-center justify-center h-8 w-12 rounded-lg bg-indigo-50 text-indigo-700 font-black italic shadow-inner border border-indigo-100">{{ $item->quantity }}</span>
                            </td>
                        ...
                        <tr class="bg-slate-50/80">
                            ...
                            <td class="py-10 pl-3 pr-10 text-right">
                                <div class="inline-flex flex-col items-end">
                                    <span class="text-[3rem] font-black text-indigo-600 leading-none italic tracking-tighter">RM {{ number_format($order->total, 2) }}</span>
                                    <span class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mt-2">Termasuk caj penghantaran & servis</span>
                                </div>
                            </td>
                        </tr>
                    </tfoot>
    ...
    <div class="space-y-8 animate-in fade-in slide-in-from-right-8 duration-700">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm ring-1 ring-slate-200 overflow-hidden transform transition-all duration-500 hover:shadow-xl">
            ...
            <div class="p-8">
                <form method="post" action="{{ route('admin.orders.update', $order) }}" class="space-y-8">
                    @csrf @method('put')
                    <div class="group">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 pl-1 group-hover:text-indigo-600 transition-colors">Tukar Status Pesanan</label>
                        <div class="relative">
                            <select name="status" class="block w-full rounded-2xl border-0 py-4 pl-5 pr-12 text-slate-900 font-black uppercase tracking-widest ring-1 ring-inset ring-slate-200 focus:ring-2 focus:ring-indigo-600 shadow-sm appearance-none cursor-pointer text-xs" required>
                    ...
                    <div class="group">
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-3 pl-1 group-hover:text-indigo-600 transition-colors">Nombor Tracking (Courier)</label>
                        <div class="relative">
                            ...
                            <input type="text" name="tracking_no" value="{{ $order->tracking_no }}" class="block w-full rounded-2xl border-0 py-4 pl-12 px-4 text-slate-900 font-black tracking-widest ring-1 ring-inset ring-slate-200 placeholder:text-slate-300 focus:ring-2 focus:ring-indigo-600 text-xs shadow-inner" placeholder="MASUKKAN NO TRACKING">
                        </div>
                    </div>

                    <button type="submit" class="w-full inline-flex items-center justify-center rounded-2xl bg-slate-900 px-6 py-5 text-sm font-black text-white shadow-xl shadow-slate-200 hover:bg-indigo-600 hover:shadow-indigo-100 transition-all active:scale-95 group/save">
                        SIMPAN PERUBAHAN
        ...
        <!-- Invoice Card -->
        <div class="relative group bg-slate-900 rounded-[2.5rem] shadow-2xl shadow-indigo-200/50 overflow-hidden text-white p-1">
            <div class="absolute inset-0 bg-gradient-to-br from-indigo-600 via-indigo-900 to-slate-900 opacity-90"></div>
            ...
            <div class="relative px-8 py-10">
                ...
                @if($order->invoice)
                    <div class="mb-10 text-center">
                        <span class="text-[10px] font-black text-indigo-300 uppercase tracking-widest block mb-1 opacity-60">Status Invois: TERJANA</span>
                    ...
                    <a href="{{ route('admin.invoices.show', $order->invoice) }}" target="_blank" class="w-full inline-flex items-center justify-center gap-3 rounded-2xl bg-white px-6 py-5 text-sm font-black text-slate-900 hover:bg-indigo-50 transition-all border-b-4 border-slate-300 active:border-b-0 active:translate-y-1">
                ...
                @else
                    <form method="post" action="{{ route('admin.invoices.store', $order) }}" class="space-y-6">
                        ...
                        <button type="submit" class="w-full inline-flex items-center justify-center gap-3 rounded-2xl bg-indigo-500 px-6 py-5 text-sm font-black text-white shadow-2xl hover:bg-indigo-400 transition-all active:scale-95">
