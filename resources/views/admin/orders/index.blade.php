    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        ...
        <form method="get" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-1.5 bg-white rounded-2xl shadow-sm ring-1 ring-slate-200">
            <div class="relative group">
                ...
                <select name="status" class="block w-full sm:w-48 rounded-xl border-0 py-2 pl-9 pr-8 text-slate-900 ring-0 focus:ring-2 focus:ring-inset focus:ring-brand-600 text-[9px] font-black uppercase tracking-[0.1em] bg-slate-50 cursor-pointer appearance-none transition-all">
        ...
        <table class="min-w-full border-separate border-spacing-0">
            <thead>
                <tr class="bg-slate-50/80">
                    <th scope="col" class="py-4 pl-6 pr-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">ID</th>
                    <th scope="col" class="px-4 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">PELANGGAN</th>
                    <th scope="col" class="px-4 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">STATUS</th>
                    <th scope="col" class="px-4 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">LOGISTIK</th>
                    <th scope="col" class="px-4 py-4 text-left text-[9px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">JUMLAH</th>
                ...
                @forelse($orders as $order)
                    <tr class="hover:bg-brand-50/20 transition-all group/row">
                        ...
                        <td class="whitespace-nowrap px-4 py-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[10px] font-bold text-slate-500 mt-0.5">{{ $order->customer_phone }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="whitespace-nowrap px-4 py-4">
                        ...
                        <td class="whitespace-nowrap px-4 py-4">
                            @if($order->tracking_no)
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] font-black text-slate-800 tracking-wider">{{ $order->tracking_no }}</span>
                            ...
                            @else
                                <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest italic">TIADA INFO</span>
                            @endif
                        </td>
                        <td class="whitespace-nowrap px-4 py-4">
                        ...
                        <td class="relative whitespace-nowrap py-4 pl-4 pr-6 text-right text-sm">
                            <a href="{{ route('admin.orders.show', $order) }}" class="inline-flex items-center justify-center rounded-xl bg-white px-4 py-2 text-[10px] font-black uppercase tracking-widest text-slate-700 shadow-sm ring-1 ring-slate-200 transition-all hover:bg-slate-900 hover:text-white active:scale-95 group/btn">
                                DETAILS
                    ...
                    <tr>
                        <td colspan="6" class="py-20 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-slate-400 font-bold uppercase tracking-widest text-[9px]">Belum ada tempahan.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
