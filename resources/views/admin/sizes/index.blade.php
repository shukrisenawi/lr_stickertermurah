        ...
        <table class="min-w-full border-separate border-spacing-0">
            <thead>
                <tr class="bg-slate-50">
                    <th scope="col" class="py-4 pl-6 pr-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">Pilihan Saiz</th>
                    <th scope="col" class="px-4 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">Dimensi</th>
                    <th scope="col" class="px-4 py-4 text-left text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">Harga Seunit</th>
                    <th scope="col" class="px-4 py-4 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100">Status</th>
                    ...
                    <tr class="hover:bg-brand-50/30 transition-colors group">
                        ...
                        <td class="whitespace-nowrap px-4 py-4">
                            <div class="inline-flex items-center gap-1.5 px-2 py-0.5 bg-slate-50 rounded-md ring-1 ring-slate-200">
                                <span class="text-[10px] font-black text-slate-600 font-mono">
                        ...
                        <td class="whitespace-nowrap px-4 py-4 text-center">
                            @if($size->is_active)
                                <span class="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest text-emerald-700 ring-1 ring-inset ring-emerald-600/20">
                                    <span class="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                    AKTIF
                                </span>
                            @else
                                <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest text-slate-500 ring-1 ring-inset ring-slate-300/30">
                        ...
                        <td class="relative whitespace-nowrap py-4 pl-4 pr-6 text-right text-sm font-medium">
                            <div class="flex justify-end items-center gap-2 transform group-hover:-translate-x-1 transition-transform duration-300">
                                <a href="{{ route('admin.sizes.edit', $size) }}" class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-[10px] font-black text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50 hover:ring-brand-300 transition-all uppercase tracking-widest">
                                ...
                                <form method="post" action="{{ route('admin.sizes.destroy', $size) }}" class="inline-block">
                                    @csrf @method('delete')
                                    <button type="submit" class="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-[10px] font-black text-rose-600 shadow-sm ring-1 ring-rose-100 hover:bg-rose-50 hover:ring-rose-300 transition-all uppercase tracking-widest" onclick="return confirm('Padam saiz ini?')">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="py-20 text-center">
                            ...
                            <p class="text-[10px] font-bold text-slate-400 italic">Mula buat konfigurasi harga.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
