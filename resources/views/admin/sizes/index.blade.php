...
@php
    ...
    $halfCount = max((int) ceil($sizeCollection->count() / 2), 1);
    $sizeChunks = $sizeCollection->chunk($halfCount);
@endphp
...
<div class="grid grid-cols-1 xl:grid-cols-2 gap-4">
    @for($cardIndex = 0; $cardIndex < 2; $cardIndex++)
        @php
            $chunk = $sizeChunks->get($cardIndex, collect());
            $cardTitle = $cardIndex === 0 ? 'Senarai Kiri' : 'Senarai Kanan';
        @endphp
        ...
        <div class="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div class="px-4 py-3 bg-slate-50 border-b border-slate-100">
                <p class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500">{{ $cardTitle }}</p>
            </div>
                ...
                <table class="min-w-full border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-slate-50">
                            ...
                            <th scope="col" class="px-3 py-3 text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] border-b border-slate-100">Status</th>
                            <th scope="col" class="relative py-3 pl-3 pr-4 border-b border-slate-100 text-right">
                        ...
                        @forelse($chunk as $size)
                            <tr class="hover:bg-brand-50/30 transition-colors group">
                                ...
                                <td class="whitespace-nowrap px-3 py-2.5 text-center">
                                    @if($size->is_active)
                                        <span class="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-emerald-700 ring-1 ring-inset ring-emerald-600/20">
                                            <span class="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                            AKTIF
                                        </span>
                                    @else
                                        <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-black uppercase tracking-wide text-slate-500 ring-1 ring-inset ring-slate-300/30">
                                ...
                                <td class="relative whitespace-nowrap py-2.5 pl-3 pr-4 text-right text-sm font-medium">
                                    <div class="flex justify-end items-center gap-1.5 transform group-hover:-translate-x-1 transition-transform duration-300">
                                        <a href="{{ route('admin.sizes.edit', $size) }}" class="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 text-[10px] font-black text-slate-700 shadow-sm ring-1 ring-slate-200 hover:bg-slate-50 hover:ring-brand-300 transition-all uppercase tracking-wide">
                                        ...
                                        <form method="post" action="{{ route('admin.sizes.destroy', $size) }}" class="inline-block">
                                            @csrf @method('delete')
                                            <button type="submit" class="inline-flex items-center gap-1 rounded-lg bg-white px-2.5 py-1.5 text-[10px] font-black text-rose-600 shadow-sm ring-1 ring-rose-100 hover:bg-rose-50 hover:ring-rose-300 transition-all uppercase tracking-wide" onclick="return confirm('Padam saiz ini?')">
