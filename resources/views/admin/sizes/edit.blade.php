...
@section('title', 'Kemaskini Saiz & Harga')
...
<div class="max-w-3xl">
    ...
    <div class="bg-white rounded-3xl shadow-sm ring-1 ring-slate-200 overflow-hidden text-sm">
        <form method="post" action="{{ route('admin.sizes.update', $size) }}" class="p-8 space-y-8">
            @csrf @method('put')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 group transition-colors hover:border-amber-200 hover:bg-amber-50/30">
                    <div class="flex h-6 items-center">
                        <input id="is_default" name="is_default" type="checkbox" value="1" class="h-5 w-5 rounded-lg border-slate-300 text-amber-600 focus:ring-amber-600 cursor-pointer" {{ old('is_default', $size->is_default) ? 'checked' : '' }}>
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-3 bg-slate-50 p-4 rounded-2xl border border-slate-100 group transition-colors hover:border-emerald-200 hover:bg-emerald-50/30">
                    <div class="flex h-6 items-center">
                        <input id="is_active" name="is_active" type="checkbox" value="1" class="h-5 w-5 rounded-lg border-slate-300 text-emerald-600 focus:ring-emerald-600 cursor-pointer" {{ old('is_active', $size->is_active) ? 'checked' : '' }}>
                    </div>
                    <div>
