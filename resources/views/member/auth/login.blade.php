...
<div class="max-w-xl mx-auto">
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-8 md:p-10">
        ...
        <h1 class="text-3xl font-black text-slate-900 tracking-tight">Login Ahli</h1>
        <p class="mt-2 text-sm text-slate-500 font-medium">Akses rekod order, invoice, dan buat repeat order dengan cepat.</p>
        ...
        <form method="post" action="{{ route('member.login.attempt') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="password" name="password" required class="w-full rounded-2xl border-slate-200 bg-slate-50 px-4 py-3.5 text-sm font-bold text-slate-900 focus:border-brand-600 focus:ring-brand-600" placeholder="Masukkan kata laluan">
            </div>
            <label class="flex items-center gap-3 text-sm font-semibold text-slate-600">
                <input type="checkbox" name="remember" class="rounded border-slate-300 text-brand-600 focus:ring-brand-600"> Kekalkan log masuk
            </label>
            <button type="submit" class="w-full rounded-2xl bg-brand-600 px-6 py-4 text-sm font-black uppercase tracking-widest text-white hover:bg-brand-700 transition-colors">Login Akaun Ahli</button>
        </form>

        <p class="mt-6 text-sm text-slate-500 font-medium">Belum ada akaun? <a href="{{ route('member.register') }}" class="font-black text-brand-600">Daftar sekarang</a>.</p>
    </div>
</div>
@endsection
