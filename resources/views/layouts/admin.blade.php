...
<body class="h-full antialiased flex overflow-hidden bg-[#f8fafc]" x-data="{ mobileSidebarOpen: false }">
    ...
    <aside :class="mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
           class="fixed inset-y-0 left-0 z-50 w-80 sidebar-premium flex flex-col transition-transform duration-500 ease-out lg:static lg:inset-auto shadow-2xl">
        <!-- Logo Section -->
        ...
        <!-- Navigation -->
        <nav class="flex-1 px-6 py-10 overflow-y-auto custom-scrollbar space-y-12">
            <!-- Group Utama -->
            <div>
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.dashboard') }}" class="sidebar-link flex items-center gap-4 py-4 px-4 rounded-[1.25rem] transition-all duration-300 {{ request()->routeIs('admin.dashboard') ? 'nav-link-active' : 'text-slate-400 hover:text-white hover:bg-white/5 group' }}">
                        ...
                        <span class="text-sm font-black uppercase tracking-widest {{ request()->routeIs('admin.dashboard') ? 'text-white' : 'text-slate-400 group-hover:text-slate-200' }}">Dashboard</span>
                    </a>
            ...
            <div>
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.categories.index') }}" class="sidebar-link flex items-center gap-4 py-4 px-4 rounded-[1.25rem] transition-all duration-300 {{ request()->routeIs('admin.categories.*') ? 'nav-link-active' : 'text-slate-400 hover:text-white hover:bg-white/5 group' }}">
                        ...
                        <span class="text-sm font-black uppercase tracking-widest {{ request()->routeIs('admin.categories.*') ? 'text-white' : 'text-slate-400 group-hover:text-slate-200' }}">Kategori</span>
                    </a>
                    ...
                    <a href="{{ route('admin.designs.index') }}" class="sidebar-link flex items-center gap-4 py-4 px-4 rounded-[1.25rem] transition-all duration-300 {{ request()->routeIs('admin.designs.*') ? 'nav-link-active' : 'text-slate-400 hover:text-white hover:bg-white/5 group' }}">
                        ...
                        <span class="text-sm font-black uppercase tracking-widest {{ request()->routeIs('admin.designs.*') ? 'text-white' : 'text-slate-400 group-hover:text-slate-200' }}">Katalog Grafik</span>
                    </a>
                    ...
                    <a href="{{ route('admin.sizes.index') }}" class="sidebar-link flex items-center gap-4 py-4 px-4 rounded-[1.25rem] transition-all duration-300 {{ request()->routeIs('admin.sizes.*') ? 'nav-link-active' : 'text-slate-400 hover:text-white hover:bg-white/5 group' }}">
                        ...
                        <span class="text-sm font-black uppercase tracking-widest {{ request()->routeIs('admin.sizes.*') ? 'text-white' : 'text-slate-400 group-hover:text-slate-200' }}">Saiz & Kos</span>
                    </a>
                </div>
            </div>
        </nav>
        ...
        <div class="p-6 border-t border-white/5">
            <form method="post" action="{{ route('admin.logout') }}">
                @csrf
                <button class="flex items-center gap-4 w-full py-4 px-6 rounded-2xl bg-rose-600/10 text-rose-400 hover:bg-rose-600 hover:text-white hover:shadow-xl hover:shadow-rose-500/20 transition-all duration-500 active:scale-95">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75" />
                    </svg>
                    <span class="text-xs font-black uppercase tracking-[0.25em]">Keluar Sistem</span>
                </button>
            </form>
        </div>
    </aside>
    ...
    <div class="flex flex-col flex-1 min-w-0 overflow-hidden relative">
        ...
        <main class="flex-1 overflow-y-auto bg-slate-50 custom-scrollbar relative z-0">
            <div class="py-12 px-8 lg:px-12 max-w-[1600px] mx-auto">
                <!-- Notifications -->
                @if(session('success'))
                    <div x-data="{ show: true }" x-show="show" x-transition class="mb-10 rounded-[1.5rem] bg-emerald-50 p-6 border border-emerald-200 shadow-xl shadow-emerald-500/5 animate-in slide-in-from-top-6">
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-4">
                                <div class="h-12 w-12 rounded-2xl bg-emerald-100 flex items-center justify-center shrink-0">
                                    <svg class="h-6 w-6 text-emerald-600" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                    </svg>
                                </div>
                                <div>
                                    <h4 class="text-sm font-black text-emerald-900 uppercase tracking-widest">Berhasil</h4>
                                    <p class="text-sm font-bold text-emerald-700/80">{{ session('success') }}</p>
                                </div>
                            </div>
                ...
                @if(session('error'))
                    <div x-data="{ show: true }" x-show="show" x-transition class="mb-10 rounded-[1.5rem] bg-rose-50 p-6 border border-rose-200 shadow-xl shadow-rose-500/5 animate-in slide-in-from-top-6">
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-4">
                                <div class="h-12 w-12 rounded-2xl bg-rose-100 flex items-center justify-center shrink-0">
                                    <svg class="h-6 w-6 text-rose-600" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z" clip-rule="evenodd" />
                                    </svg>
                                </div>
                                <div>
                                    <h4 class="text-sm font-black text-rose-900 uppercase tracking-widest">Ralat Sistem</h4>
                                    <p class="text-sm font-bold text-rose-700/80">{{ session('error') }}</p>
                                </div>
                            </div>
                ...
                <!-- Yield Content -->
                <div class="animate-in fade-in slide-in-from-bottom-8 duration-1000 fill-mode-both">
                    @yield('content')
                </div>
            </div>
        </main>
